...
public sealed class PersistenceDetector : IPersistenceDetector
{
    ...
    {
        var entries = new global::System.Collections.Generic.List<AutostartEntry>();
    ...
    public IReadOnlyList<AutostartEntry> ScanForNew(IReadOnlyList<AutostartEntry> baseline)
    {
        ...
        var baselineSet = new global::System.Collections.Generic.HashSet<string>(
            baseline.Select(e => $"{e.Location}|{e.Name}|{e.Value}"));
        return current.Where(e => !baselineSet.Contains($"{e.Location}|{e.Name}|{e.Value}")).ToList();
    }

    private static void ScanRegistryAutostart(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
    ...
    private static void ScanWinlogon(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
        ...
        try
        {
            ...
            foreach (var name in interestingValues)
            {
                ...
                {
                    "Shell" when !value.Equals("explorer.exe", global::System.StringComparison.OrdinalIgnoreCase) => 80,
                    "Userinit" when !value.Contains("userinit.exe", global::System.StringComparison.OrdinalIgnoreCase) => 80,
                    _ => 10
                };
    ...
    private static void ScanImageFileExecutionOptions(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
        try
        {
    ...
    private static void ScanStartupFolders(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
        string[] startupPaths =
        [
            ...
            global::System.Environment.GetFolderPath(global::System.Environment.SpecialFolder.CommonStartup)
        ];

        foreach (var folder in startupPaths)
        {
            if (!global::System.IO.Directory.Exists(folder)) continue;

            try
            {
                foreach (var file in global::System.IO.Directory.GetFiles(folder))
                {
                    string ext = global::System.IO.Path.GetExtension(file).ToLowerInvariant();
                    int risk = ext switch
                    {
    ...
    private static void ScanServices(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
        try
        {
            ...
            foreach (var serviceName in servicesKey.GetSubKeyNames())
            {
                try
                {
                    ...
                    if (imagePath.Contains(@"\Windows\", global::System.StringComparison.OrdinalIgnoreCase)
                        && !imagePath.Contains(@"\Temp\", global::System.StringComparison.OrdinalIgnoreCase))
                        continue;

                    int risk = start == 0 ? 30 : 20;
                    if (!imagePath.Contains(@"\Program Files", global::System.StringComparison.OrdinalIgnoreCase))
                        risk += 20;
    ...
    private static void ScanScheduledTasks(global::System.Collections.Generic.List<AutostartEntry> entries)
    {
        try
        {
            ...
            while (reader.ReadLine() is { } line)
            {
                ...
                if (taskName.StartsWith(@"\Microsoft\", global::System.StringComparison.OrdinalIgnoreCase))
                    continue;

                if (string.IsNullOrWhiteSpace(action) || action == "N/A") continue;

                int risk = 25;
                if (action.Contains(@"\Temp\", global::System.StringComparison.OrdinalIgnoreCase)
                    || action.Contains(@"\AppData\", global::System.StringComparison.OrdinalIgnoreCase))
                    risk = 50;
    ...
    private static int ScoreRegistryEntry(string value, string keyPath)
    {
        ...
        return global::System.Math.Min(score, 100);
    }
}
